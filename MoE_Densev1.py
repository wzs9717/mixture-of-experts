...
def main():
    ...
    model.summary()
    ##train model
    opt = keras.optimizers.SGD(lr=1e-7, decay=1e-5)
    model.load_weights("weights/Denlayer.h5")
    print("Model loaded.")

    lr_reducer= ReduceLROnPlateau(monitor='val_loss', factor=np.sqrt(0.1),
                                      cooldown=0, patience=10, min_lr=0.5e-6)
    early_stopper= EarlyStopping(monitor='val_acc', min_delta=0.0001, patience=20)
    model_checkpoint= ModelCheckpoint("weights/Denlayer.h5", monitor="val_acc", save_best_only=True,
